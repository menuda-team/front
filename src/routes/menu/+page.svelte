<script lang="ts">
	...
	const eventsQueue = AsyncQueue.channels(1)
		.process(eventsHandler)
		.failure((err, task) => console.error(`failure: ${err} ${task.category}`));

	let first = true;
	const categoryVisibleEventHandler = debounce(
		({ detail }) => {
...
<div class="relative">
	...
	{#if $cart.totalAmount > 0}
		<div class="h-[80px]" in:slide out:slide />
...
{#if openedProduct}
	<div class="fixed bg-black1 opacity-25 h-screen w-full z-10 top-0 touch-none" on:click={backToMenu} />
{/if}

{#if openedProduct}
	<Swipable classes="fixed bottom-0 z-20" onClose={backToMenu}>
		<div>
			<div class="w-[50px] h-[5px] mb-2 bg-white rounded-sm mx-auto" />
